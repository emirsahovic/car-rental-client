...
const Vehicles = () => {
    ...
    const [countType, setCountType] = useState<CountProps[]>([]);
    ...
    useEffect(() => {
        ...
        const countAll = async () => {
            ...
            const res = await apiService.get('/car/countBy');

            if (mounted) setCountType(res.data);
            ...
            <Container maxWidth='xl' sx={{ mb: '7rem' }}>
                ...
                <Stack direction={{ xs: 'column', md: 'row', }} justifyContent='space-between' spacing={{ md: 3 }} marginBottom={10}>
                    <Box marginTop={4}>
                        ...
                        <Box sx={{ backgroundColor: 'rgb(250, 250, 250)', mt: '2.4rem', display: 'block', mx: 'auto', width: { xs: '70%', sm: '45%', md: '18.5rem' }, height: (displayCategory && displayColor && displayFuel) ? '48rem' : (!displayCategory && !displayColor && !displayFuel) ? '15rem' : (displayFuel && !displayCategory && !displayColor) ? '20rem' : (displayFuel && displayColor && !displayCategory) ? '32.5rem' : (displayCategory && displayColor && !displayFuel) ? '43rem' : (displayCategory && displayFuel && !displayColor) ? '34.5rem' : '28.5rem', p: '0rem 1.7rem', borderRadius: '7px', border: '1px solid #B7B5B566', boxShadow: '0px 6px 18px -15px #111', }}>
                                ...
                                <>
                                    {categories && categories.map((c: Category) => (
                                        <Box key={c._id} marginTop={1.5} display='flex' justifyContent='space-between' alignItems='center' className='checkboxPadding'>
                                            ...
                                            {countType && countType.map((item: CountProps) => (
